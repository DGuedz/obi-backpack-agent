...
OBI includes a specialized Web Interface (Next.js) that serves as the **Mission Control Center** for the agent. It is not just a landing page; it is a functional dashboard for monitoring performance and validating on-chain proofs.

###  Core Modules
1.  **Command Center:** Real-time visualization of agent metrics (Volume, PnL, Active Positions).
2.  **Audit Vault:** A transparent interface to verify the cryptographic SHA256 receipts generated by the VSC engine.
3.  **License Forge:** Integration with AgentWallet to mint and manage access licenses on Solana/Base.

###  How to Run (Local)
1.  Navigate to the web module: `cd obiwork_web`
2.  Install dependencies: `npm install`
3.  Start the server: `npm run dev`
4.  Access: `http://localhost:3000`

###  Deploy to Vercel (For Judges)
The interface is optimized for Vercel. To deploy the live dashboard:

1.  Fork this repository.
2.  Import the project into Vercel.
3.  **Important:** Set the `Root Directory` to `obiwork_web` in the Vercel Project Settings.
4.  Click **Deploy**.
